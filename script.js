...
const savedMode = localStorage.getItem("coino-mode");
...
if (toggleButton) {
  ...
  toggleButton.addEventListener("click", () => {
    ...
    localStorage.setItem("coino-mode", body.classList.contains("dark") ? "dark" : "light");
    syncToggleText();
  });
}
...
[monthBudget, needSpend, wantSpend, saveSpend].forEach((el) => {
  if (el) el.addEventListener("input", updateBudgetPlanner);
});
updateBudgetPlanner();
...
state.insightFilters = [
  { id: nextInsightId++, name: "Food", amount: 35, active: true },
  { id: nextInsightId++, name: "Transport", amount: 40, active: true },
  { id: nextInsightId++, name: "School", amount: 25, active: true }
];
state.goals = [{ name: "Headphones", amount: 350, saved: 80, weekly: 25 }];
renderGoals();
renderInsightFilterRows();
renderExpenses();
